...
class Command(BaseCommand):
    help = '''
    Checks the current day for tasks and then 
    notifies household members of their tasks for the day
    '''
    ...
    def handle(self, *args, **kwargs):
        ...
        todays_weekly_chores = AssignedTask.objects.filter(
                day__week=this_week,day=today,frequency=2
                )
        todays_daily_chores = AssignedTask.objects.filter(
                day__week=this_week,day=today,frequency=1
                )
        ...
        for i in todays_weekly_chores:
            ...
            try:
                response = client.chat_postMessage(
                        channel=i.owner.slack_memberid, text=message
                        )
            except SlackApiError as e:
                assert e.response["error"]
